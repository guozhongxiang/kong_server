...
{
    static $class = array();
    if(!array_key_exists($class_name, $class) || !is_object($class[$class_name]))
    {
        $class[$class_name] = new $class_name;
    }
    return $class[$class_name];
}

/**
 * [GetNumberRand 获取随机数字]
 * @param [int] $number [随机的位数]
 * @return[int]         [随机数]
 */
function GetNumberRand($number = 6)
{
    if(empty($number)) $number = 6;

    $rand = '';
    for($i=0; $i<$number; $i++) $rand .= rand(0, 9);
    return $rand;
}

/**
 * [R 获取所有请求的数据]
 * @param  [string] $key            [指定索引名称]
 * @param  [mixed]  $default        [默认值]
 * @return [string|array|空字符串]   [用户请求的数据]
 */
function R($key = '', $default = '')
{
    /* 处理各种场景的请求数据 */
    $raw_data = empty($HTTP_RAW_POST_DATA) ? array() : (Is_Json($HTTP_RAW_POST_DATA) ? json_decode($HTTP_RAW_POST_DATA, true) : array());
    $request = empty($_REQUEST) ? array() : $_REQUEST;
    $input_data = file_get_contents("php://input");
    $input_data = empty($input_data) ? array() : (Is_Json($input_data) ? json_decode($input_data, true) : array());
    $data = array_merge($raw_data, $request, $input_data);
...
function Xml_Parser($xml)
{
    ...
    $str = json_decode(json_encode(simplexml_load_string($xml)),true);
    return !empty($str);
}
...
function Sms_Notice_Send($content, $mobile_phone)
{
    ...
    $post = array(
      'apikey'  =>  '87137e9bbfd47295d39b46e23cdcf1d9',
      'text'    =>  '【饭高科技】'.$content.'回T退订',
      'mobile'  =>  $mobile_phone,
    );
...
{    
    $post = array(
      'apikey'  =>  '544a58ea2bac2afd3ff1f7c9d223012e',
      'text'    =>  '【饭高科技】'.$content,
      'mobile'  =>  $mobile_phone,
    );
    $result = json_decode(Fsockopen_Post('http://yunpian.com/v1/sms/send.json', $post), true);
...
/**
 * [Voice_Code_Send description 语音验证码通道]
 * @param   [string] $code         [验证码]
 * @param   [string] $mobile_phone [手机号码]
 * @return  [boolean]              [成功true, 失败false]
 */
function Voice_Code_Send($code, $mobile_phone)
{
    if(!MobilePhoneFormatCheck($mobile_phone) || empty($code)) return false;
    ...
    $result = json_decode(Fsockopen_Post('http://voice.yunpian.com/v1/voice/send.json', $post), true);
    if(empty($result)) return false;
    return ($result['msg'] == 'OK');
}
...
{
    return preg_match('/^1((3|4|5|7|8){1}\d{1})\d{8}$/', $mobile_phone);
}
...
function tip_message($msg= '',$goto="",$delay = 0, $color = '#FFFFDD'){
    ...
    echo '<html xmlns="http://www.w3.org/1999/xhtml">';
    echo '<head>';
    echo '<meta name="viewport" content="initial-scale=1, maximum-scale=1, minimum-scale=1, user-scalable=no, minimal-ui">';
    echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
    if($goto != ""){
        if($goto == 'back'){
    ...
    echo '<div style="width:100%;margin:0 auto;text-align:center; background-color:'.$color.';border: 1px solid #E0E0E0;-webkit-box-sizing:border-box;box-sizing: border-box;">'.$msg;
    echo '<br /><br />'.$delay.'秒后自动关闭';
    if($goto != "" && $goto != 'back') echo '，<a href="'.$goto.'">点此继续</a>';
    echo '</div>';
    echo '</body>';
    echo '</html>';
    exit;
...
function echo_message($data,$color){
    if(defined('APP_NAME') && APP_NAME == 'www'){
 ...
 * @return void
 */
function create_full_folder($path) {
    if(preg_match("/\/$/",$path))
...
function json_success($data='',$callback = ''){
    $result = array(
        'status' => 1,
        'statusText' => is_string($data)? $data : '操作成功',
        'data' => $data
    );
...
function get_varray_from_array2($array2,$value = 'id'){
    if(empty($array2) || !is_array($array2)) return array();
...
function get_client_ip($long = false)
{
    ...
    if($long) $onlineip = sprintf("%u", ip2long($realip));
    return $onlineip;
}
