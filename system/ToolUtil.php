...
if (!defined("PHPLIB_ROOT")) {
	define('PHPLIB_ROOT', '/data/release/system/');
}
...
abstract class ToolUtil
{
	...
	public static function checkMobilePhone($phone, $type = 'CHN')
	{
		...
		switch ($type) {
			case "CHN":
				$ret = (preg_match("/^((\(\d{3}\))|(\d{3}\-))?((13\d)|(18\d)|(17\d)|(15[^4])|(14[57]))\d{8}$/", trim($phone)) ? true : false);
				break;
			case "INT":
     ...
     * @return        bool        正确返回true, 错误返回false
     */
    public static function checkTelephone($phone, $type = 'CHN'){
       return self::checkPhone($phone, $type)|| self::checkMobilePhone($phone, $type);
    }
	...
	public static function checkLoginOrRedirect()
	{
		...
		if ($uid === false || $uid == 0) {
			...
			self::redirect('https://base.'.$domainName.'/login.html?url=' . urlencode($currentUrl));
			return 0;
		}
	...
	public static function gbJsonEncode($data)
	{
		...
		if (is_array($data)) {
			...
			//$data = json_encode(iconv("GBK", "UTF-8//IGNORE", $data));
            $data =json_encode(mb_convert_encoding( $data, 'UTF-8', 'GBK'));
		} else {
	...
	public static function checkInput($str, $length, $filter = true, $dirty = false)
	{
		...
		require_once PHPLIB_ROOT . 'Dirty.php';
		if ($dirty) {
	...
	*/
	...
	/**
	 * 根据配置部分输出目标数组中的值，可以做多维配置
	 * @param $obj 目标数组
	 * @param $mask 输出配置
	 * @param $allowEmpty 是否允许目标数组中未定义配置中的值
	 * @example
	 * $obj = array(
	'k1' => 'v1',
	'k2' => array(
	...
	public static function getImageSize($url)
	{
		...
		for($i = 0; $i < 6; $i++) {
		    ...
		    // 可能因为$data_size太小没有获取到长宽信息，加大$data_size重试
			$data_size *= 4;
		}
	...
	/**
	 * 获取区ID
	 * @param string $cookie COOKIE 中的地区信息
	 * @return int
	 */
	public static function getCommonPrid($cookie) {
		$prid_provinceid = explode('_', $cookie); //地址cookies: '$prid_$provinceid'
		$prid = $prid_provinceid[0]; //区id
     ...
     *         2: 解析数据出错
     *         3：共享内存占满
     * ps:使用需要机器安装itil.so扩展
     */
    public static function itilReport($attId, $increase = 1)
    {
   ...
   /**
     * 第一个参数是你要生成优惠码的个数（在这里是生成50个）。第二个参数exclude array，确保在当前列表中的生成唯一优惠码，所以如果你已经数据库中有一些未使用的代码，你可以把它传递给exclude。最后一个参数是优惠码的的长度。这个函数将返回规定长度的优惠码 这里是4位的优惠码。这里我已经使用数字和大写字母组合，赋值给$characters的字符串，你可以使用小写字母或任何其他的字母组合试用
	 * @param int $no_of_codes//定义生成优惠码个个数
	 * @param array $exclude_codes_array//
	 * @param int $code_length//优惠码的长度
	 * @param string $prefix//优惠码的前缀
	 * @return array //返回数组
	 */
	public static function generate_promotion_code($no_of_codes,$exclude_codes_array='',$code_length = 8,$prefix = ""){
	...
	public static function echoByEclub($callback = "", $result) {
		if (isset($callback) && !empty($callback)) {
			$callBack = ToolUtil::transXSSContent(trim($callback));
	...
	public static function luosimao($mobile, $message) {
		$ch = curl_init();
		...
		curl_setopt($ch, CURLOPT_USERPWD  , 'api:key-6d9e84958c54ab0cd891ded637e7b0a4');
